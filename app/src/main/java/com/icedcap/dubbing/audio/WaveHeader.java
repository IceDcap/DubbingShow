...
public class WaveHeader {
    ...
    public byte[] getHeader() {
        final long totalDataLen = 36l + (long) mNumBytes;
        final int numChannels = mNumChannels;
        final long byteRate = numChannels * mSampleRate * mBitsPerSample / 8;
        final long sampleRate = mSampleRate;
        final int numBytes = mNumBytes;
        ...
        header[38] = 't';
        header[39] = 'a';
        header[40] = (byte) (numBytes & 0xff);
        header[41] = (byte) ((numBytes >> 8) & 0xff);
        header[42] = (byte) ((numBytes >> 16) & 0xff);
        header[43] = (byte) ((numBytes >> 24) & 0xff);
        return header;
    }
